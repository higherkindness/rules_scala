 ...
 *
 * Zinc has two caches:
 *   1. a ClassLoaderCache which is a soft reference cache for classloaders of Scala compilers. 2. a CompilerCache which
 *      is a hard reference cache for (I think) Scala compiler instances.
 *
...
object ZincRunner extends WorkerMain[Namespace] {
  ...
  protected[this] def work(worker: Namespace, args: Array[String], out: PrintStream) = {
    ...
    val compilers = {
      ...
      ZincUtil.compilers(scalaInstance, ClasspathOptionsUtil.auto, None, scalaCompiler)
    }
